...
Fixpoint oddmembers (l:natlist) : natlist :=
  match l, even (hd 0 l) with
  | nil, _ => nil
  | h :: l', true => oddmembers l'
  | h :: l', false => h:: oddmembers l'
  end.
...
Fixpoint count (v : nat) (s : bag) : nat :=
  match s, (v =? hd v s) with
  | nil, _ => 0
  | _ :: s', false => count v s'
  | _ :: s', true => S (count v s')
  end.
...
Fixpoint remove_one (v : nat) (s : bag) : bag :=
  match s, v =? hd 0 s with
  | nil, _ => nil
  | h :: t, true => t
  | h :: t, false => h :: remove_one v t
  end.
...
Fixpoint remove_all (v:nat) (s:bag) : bag :=
  match s, v =? hd 0 s with
  | nil, _ => nil
  | h :: t, true => remove_all v t
  | h :: t, false => h :: remove_all v t
  end.
...
Fixpoint subset (s1 : bag) (s2 : bag) : bool :=
  match s1, (count (hd 0 s1) s2 =? 0) with
  | nil, _ => true
  | _, true => false
  | h :: t, false => subset t (remove_one h s2)
  end.
...
Theorem eqb_for_self : forall (n:nat), n =? n = true.
Proof.
...
Theorem bag_theorem : forall (v:nat) (s:bag),
    S (count v s) =? count v (add v s) = true.
Proof.
  intros v s. simpl.  rewrite eqb_for_self. rewrite eqb_for_self. reflexivity.
Qed.
...
Fixpoint eqblist (l1 l2 : natlist) : bool :=
  ...
  | h1::l1', nil => false
  | nil, h2::l2' => false
  | h1::l1', h2::l2' => andb (h1 =? h2) (eqblist l1' l2')
  end.
